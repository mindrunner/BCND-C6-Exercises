...
contract('ExerciseC6D', async (accounts) => {
    ...
    before('setup contract', async () => {
        ...
        const oracleRequest = await config.exerciseC6D.OracleRequest();
        const flightStatusInfo = await config.exerciseC6D.FlightStatusInfo();

        oracleRequest.on("data", (data) => {
            console.log(`\n\nOracle Requested: index: ${data.args.index.toNumber()}, flight:  ${data.args.flight}, timestamp: ${data.args.timestamp.toNumber()}`);

        });
        flightStatusInfo.on("data", (data) => {
            console.log(`\n\nFlight Status Available: flight: ${data.args.flight}, timestamp: ${data.args.timestamp.toNumber()}, status: ${data.args.status.toNumber() == ON_TIME ? 'ON TIME' : 'DELAYED'}, verified: ${data.args.verified ? 'VERIFIED' : 'UNVERIFIED'}`);
        });
